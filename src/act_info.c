...
char *	const	where_name	[] =
{
    ...
    "#4[#3Left Hand     #4]#n ",
    "#4[#3On Face       #4]#n ",
    "#4[#3Left Scabbard #4]#n ",
    "#4[#3Right Scabbard#4]#n ",
};
...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    buf[0] = '\0';
    if ( IS_SET(obj->quest, QUEST_ARTIFACT)) strcat(buf, "#C(#LA#lrtifact#C)#8 " );
    else if ( IS_SET(obj->quest, QUEST_ALIEN)) strcat(buf, "#c(#GA#glien#c)#n " );
    else if ( IS_SET(obj->quest, QUEST_MYSTIC)) strcat(buf, "#c(#yM#oystical#c)#n " );   
    else if ( IS_SET(obj->quest, QUEST_RELIC)) strcat(buf, "#1(#RR#relic#1)#8 " );
    else if ( obj->points > 1200 ) strcat(buf, "(Mythical) " );
    else if ( obj->points > 600 ) strcat(buf, "(Legendary) ");
    else if ( obj->points > 0 ) strcat(buf, "(Enhanced) ");
...
/* mog */
	...
	if (IS_CLASS(ch,CLASS_VAMPIRE) && ch->pcdata->stats[UNI_RAGE] > 0)
	    ...
	    hr += 1000;
	else if (IS_CLASS(ch, CLASS_DROWNEW) && IS_SET(ch->xkilla->plr, PLR_TOUGHEN))
{	    hr += 250; hr += (ch->xkilla->dskills * 10);}
...
{            dr += 250; dr += (ch->xkilla->dskills * 10);   }
	else if (IS_CLASS(ch, CLASS_HIGHLANDER) && (get_eq_char(ch, WEAR_WIELD) != NULL))
/* &&
IS_SET(obj->spectype,
SITEM_HIGHLANDER))*/
...
{            a_c -= 500; a_c -= (ch->xkilla->dskills * 20);   }
	else if (IS_CLASS(ch, CLASS_PALADIN)) 
	{
		a_c -= 1000;
	}
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if (!IS_NPC(victim) && victim->desc==NULL ) strcat( buf,"(Link-Dead) ");
	 ...
	 strcat( buf, "(Translucent) ");
    	if ( IS_AFFECTED(victim, AFF_FAERIE_FIRE) ) strcat( buf, "#5(Pink Aura) #8");
    	if ( IS_EVIL(victim)
    	&&   IS_AFFECTED(ch, AFF_DETECT_EVIL)     ) strcat( buf, "#1(Red Aura) #8");
    	if ( IS_AFFECTED(victim, AFF_SANCTUARY)   ) strcat( buf, "#7(White Aura)#n  ");

 	if ( IS_AFFECTED2(victim, AFF_DEMONFIRE)) strcat( buf, "#1(LavaShield) #8");
    }
    ...
    if (IS_NPC(victim) &&ch->questmob > 0 && victim->pIndexData->vnum == ch->questmob)
        strcat( buf, "#2[#1#b#uTARGET#n#2]#n ");
    if ( IS_SET(ch->tag_flags,TAG_RED) ) strcat( buf,"#1[RED] #n"  );
    if ( IS_SET(ch->tag_flags,TAG_BLUE)) strcat( buf,"#4[BLUE] #n" );
/* Simple CLass telling Xkilla*/
if ( !IS_NPC(ch) && (ch->class > 0 || IS_IMMORTAL(ch)) )
    {
...
/*why do we need dark flags in a pk mud? doesn't make much of a diff,
people don't log on to a gw mud to look at the quality of the rooms and
they do not want to see "it is pitch blach ..." all the time
    if ( !IS_NPC(ch)
    &&   !IS_SET(ch->act, PLR_HOLYLIGHT)
    &&   !IS_ITEMAFF(ch, ITEMA_VISION)
...
*/
	...
	else if ( ( !IS_NPC(ch) && !IS_SET(ch->act, PLR_BRIEF) ) && (ch->desc && !ch->desc->run_buf) &&
		( arg1[0] == '\0' || !str_cmp( arg1, "auto" ) ) )
	{
            sprintf(buf, "#4%s#n", ch->in_room->description);
            send_to_char( buf, ch );
...
void do_exits( CHAR_DATA *ch, char *argument )
{
    ...
    strcpy( buf, fAuto ? "#0[Exits:" : "#0Obvious exits:#n\n\r" );
    ...
    if ( !found )
	strcat( buf, fAuto ? " #7none#n" : "#7None#n.\n\r" );
...
IS_SET(ch->special,SPC_CHAMPION)))
    ...
    */
    if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_ALIEN)) {
        sprintf( buf, "You have %d planet points!.\n\r",
...
ch);
    ...
    else if (!IS_NPC(ch) && ch->race <= 19) send_to_char( "an Immortal.\n\r", ch);
    else if (!IS_NPC(ch) && ch->race <= 39) send_to_char( "a Godling.\n\r", ch);
    else if (!IS_NPC(ch) && ch->race <= 59) send_to_char( "a Demigod.\n\r", ch);
    else if (!IS_NPC(ch) && ch->race <= 79) send_to_char( "a Lesser God.\n\r",
ch);
    else if (!IS_NPC(ch) && ch->race <= 99) send_to_char( "a Greater God.\n\r",
ch);
    else if (!IS_NPC(ch) && ch->race >= 100) send_to_char( "a Supreme God.\n\r",
ch);
...
ss2 );
    ...
    if (IS_AFFECTED(ch,AFF_HIDE)) send_to_char( "You are keeping yourself hidden
from those around you.\n\r", ch );
	...
	else if ( IS_CLASS( ch, CLASS_DEMON) && ch->pcdata->stats[DEMON_POWER] >
0)
	{
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	"It is %d o'clock %s, Day of %s, %d%s the Month of %s.\n\rTHe RoK started up at %s\rThe system time is %s\r",
...
if (!str_cmp(wch->name,"Tanais")) class = "#gK#1i#3N#LG #2           ";
else if (!str_cmp(wch->name,"Serenity")) class = "   #b#i#yMoon#n #b#i#PQueen#n   ";
            else if (!str_cmp(wch->name,"Xkilla")) class = "#i#1Coder           #n";
                       else class = "Implementor     "; break;  
	case MAX_LEVEL -  1: 
if (!str_cmp(wch->name,"")) class =      "#7**************#n  ";
                else class =         "#4High Judge      #n"; break;
	case MAX_LEVEL -  2: class = "#5Judge          #n "; break;
	case MAX_LEVEL -  3: class = "#6Police      #n    "; break;
	case MAX_LEVEL -  4: class = "#3Helper  #n        "; break;
	case MAX_LEVEL -  5: class = "#2Builder   #n      "; break;
	case MAX_LEVEL -  6:
	case MAX_LEVEL -  7:
	case MAX_LEVEL -  8:
	case MAX_LEVEL -  9:
		     if (wch->race <= 0 ) class = "#1Kalos   #n        ";
		else if (wch->race <= 3 ) class = "#2Killer#n          ";
		else if (wch->race <= 6 ) class = "#3Apprentice#n      ";
		else if (wch->race <= 9 ) class = "#4Fighter#n         ";
		else if (wch->race <= 12) class = "#5Warrior#n         ";
...
if ( IS_CLASS(wch, CLASS_VAMPIRE))
	...
	{ strcpy( openb, " *-|" ); strcpy( closeb, "|-*" ); }
	else if ( IS_CLASS(wch, CLASS_SHATAN))
        { strcpy( openb, "=:" ); strcpy( closeb, "=:" ); }
	else if ( IS_CLASS(wch,CLASS_MAGE))
        { strcpy( openb, "{" ); strcpy( closeb, "}" ); }
        else if ( IS_CLASS(wch, CLASS_HIGHLANDER))
        { strcpy( openb, "-=" ); strcpy( closeb, "=-" ); }
	else if ( IS_CLASS(wch, CLASS_DEALER))
	{ strcpy( openb, "=-" ); strcpy( closeb, "-="); }
        else if ( IS_CLASS(wch, CLASS_ALIEN))
        { strcpy( openb, ";|" ); strcpy( closeb, "|;" ); }
        else if ( IS_CLASS(wch, CLASS_NINJA))
        { strcpy( openb, "**" ); strcpy( closeb, "**" ); }
        else if ( IS_CLASS(wch, CLASS_REVNANT))
        { strcpy( openb, "<:" ); strcpy( closeb, ":>" ); }
        else if ( IS_CLASS(wch, CLASS_PALADIN))
        { strcpy( openb, "(:" ); strcpy( closeb, ":)" ); }
        else if ( IS_CLASS(wch, CLASS_DROWNEW))
        { strcpy( openb, ":-= "); strcpy(closeb, " =-:" ); }
        else if ( IS_CLASS(wch, CLASS_DRAGON))
  { strcpy( openb, "-+- "); strcpy(closeb, " -+-" ); }
        else if (ch->class > 0 || IS_IMMORTAL( ch ))
        { strcpy( openb, "[" ); strcpy( closeb, "]" ); }
	  {
		...
		if (IS_SET(wch->special, SPC_PRINCE) && wch->sex == SEX_FEMALE)
		    sprintf( kav, ". %sDemon Princess of %s%s", openb,wch->lord,closeb );
		else if (IS_SET(wch->special, SPC_PRINCE))
		    sprintf( kav, ". %sDemon Prince of %s%s", openb,wch->lord,closeb);
		else if (IS_SET(wch->special, SPC_SIRE))
		    sprintf( kav, ". %sDemon%s", openb,closeb );
		else
		    sprintf( kav, ". %sChampion of %s%s", openb,wch->lord,closeb );
	    }
else if (IS_CLASS(wch, CLASS_DROWNEW) && wch->pcdata->stats[UNI_GEN] == 1 ) 
                sprintf( kav, " %sMaster spider%s", openb, closeb );
            ...
            else if (IS_CLASS(wch, CLASS_ALIEN) && wch->pcdata->stats[UNI_GEN] == 1 )
                sprintf( kav, ". %sAlien Drone%s", openb, closeb );
...
//rigged monks
                if (wch->pcdata->powers[PMONK] > 18)
		    strcat(kav, " is a cheater!.");
            }
            else if ( IS_CLASS(wch, CLASS_REVNANT)
                && wch->clan != NULL && strlen(wch->clan) < 1 )
	    {
                sprintf( kav, ". %sFree Revnant%s", openb, closeb );
//rigged monks
                if(wch->pcdata->powers[PMONK] > 18)
		    strcat(kav, " is a cheater!.");
	    }
            ...
            {
                if (wch->pcdata->stats[UNI_GEN] == 2)
                    sprintf( kav, ". %s%s Black Widow%s", openb, wch->clan, closeb );
                else if (IS_SET(wch->special, SPC_PRINCE))
                    sprintf( kav, ". %s%s Prince%s", openb, wch->clan, closeb );
                else
                    sprintf( kav, ". %s%s Drow%s", openb, wch->clan, closeb );
            }
            else if ( IS_CLASS(wch, CLASS_DROWNEW)
                && wch->clan != NULL && strlen(wch->clan) < 1 )
            ...
            {
                if (wch->pcdata->stats[UNI_GEN] == 2)
                    sprintf( kav, ". %s%s Cyber-OverLord%s", openb, wch->clan, closeb );
                else if (IS_SET(wch->special, SPC_PRINCE))
                    sprintf( kav, ". %s%s CyberAssassin%s", openb, wch->clan, closeb );
                else
                    sprintf( kav, ". %s%s CyberSoldier%s", openb, wch->clan, closeb );
            }
            else if ( IS_CLASS(wch, CLASS_NINJA)
                && wch->clan != NULL && strlen(wch->clan) < 1 )
                sprintf( kav, ". %sAssassin for Hire%s", openb, closeb );
	   else if ( IS_CLASS(wch, CLASS_SHATAN)
                && wch->clan != NULL && strlen(wch->clan) < 1 )
                sprintf( kav, ". %sShatan Follower%s", openb, closeb );
	    else if ( IS_CLASS(wch, CLASS_HYDRA)
		&& wch->clan != NULL && strlen(wch->clan) < 1 )
		sprintf( kav, ".[%d]%sHydra%s",wch->pcdata->powers[DISC_HYDRA_HEADS],openb,closeb );	    
	    else if ( IS_CLASS(wch, CLASS_WEREWOLF)
		&& wch->clan != NULL && strlen(wch->clan) > 1 )
	    {
	    	if (wch->pcdata->stats[UNI_GEN] == 2)
		...
		&& wch->clan != NULL && strlen(wch->clan) < 1 )
            ...
            {
                if (wch->pcdata->stats[UNI_GEN] == 2)
                    sprintf( kav, ". %s%s Hunter%s", openb, wch->clan, closeb );
                else if (IS_SET(wch->special, SPC_PRINCE))
                    sprintf( kav, ". %s%s Stalker%s", openb, wch->clan, closeb );
                else
                    sprintf( kav, ". %s%s Invader%s", openb, wch->clan, closeb );
            }
            else if ( IS_CLASS(wch, CLASS_ALIEN)
                && wch->clan != NULL && strlen(wch->clan) < 1 )
                sprintf( kav, ". %sNomad%s", openb, closeb );
	    else if ( IS_CLASS(wch, CLASS_MAGE) )
	    {
		...
		if (wch->level == LEVEL_APPRENTICE)
            ...
            {
                sprintf( kav, ". %sDrug Dealer%s", openb, closeb);
            }
            else if ( IS_CLASS(wch, CLASS_PALADIN) )
            {
	...
	if ( wch->level > 6 )
        {
	sprintf( immo + strlen(immo), "#1[%s#1]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
	...
	imm++;
      ...
      else if ( wch->level > 2 && wch->level < 7 )
      {
        if ( wch->race == 0 )
        {
          sprintf( a0 + strlen(a0), "#1[%s#1]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
        ...
        {
	  sprintf( a1 + strlen(a1), "#1[%s#1]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
        ...
        {
	  sprintf( a2 + strlen(a2), "#1[%s#1]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
        ...
        {
	  sprintf( a3 + strlen(a3), "#1[%s#1]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
        ...
        {
	  sprintf( a4 + strlen(a4), "#1[%s]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
        ...
        {
	  sprintf( a5 + strlen(a5), "#1[%s#1]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
        ...
        {
	  sprintf( a6 + strlen(a6), "#1[%s#1]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
        ...
        {
	  sprintf( a7 + strlen(a7), "#1[%s]#n #2%s%s#n#6%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
      ...
      {
        sprintf( mort + strlen(mort), "#1[%s#1]#n #2%s%s#n#3%s#n#4%s#n\n\r",
	    class,
            wch->pcdata->pretit,
	    wch->name,
          wch->pcdata->title,
...
send_to_char("#0-------------------==========#u#1 Revenge#2 of #3Kalos #n#0==========-------------------#n\n\r",ch);

    if ( imm > 0 )
    {
     if ( imm > 1 )
      sprintf(buf, "#7----------------------------#1 %d Immortals #7----------------------------#n\n\r",imm
);
     else
      sprintf(buf, "#7----------------------------#2 1 Immortal #7----------------------------#n\n\r");
     send_to_char(buf,ch); 
     send_to_char(immo,ch);
    }

    if ( ava > 0 )
    {
      if ( ava > 1 )
       sprintf(buf, "#7-----------------------------#3 %d Avatars #7----------------------------#n\n\r",ava);
      else
       sprintf(buf, "#7-----------------------------#4 One Avatar #7----------------------------#n\n\r");
      send_to_char(buf,ch);
      if ( a7[0] != '\0' )
        send_to_char(a7,ch);
      if ( a6[0] != '\0' )
    ...
    {
      if ( mor > 1 )
       sprintf(buf, "#7----------------------------#5 %d Mortals#7----------------------------#n\n\r",mor);
      else
       sprintf(buf, "#7----------------------------#6 One Mortal #7----------------------------#n\n\r");
      send_to_char(buf,ch);
      send_to_char(mort,ch);
    }

if (nMatch == 1 && total == 0)
       sprintf( buf2, "#4There are a total of #u#21#n #4visible players connected.#n \n\r");
    else if (nMatch == 1 && total != 0)
        sprintf(buf2, "#4There are a total of #u#21#n #4visible player out of #2%d total players connected.#n\n\r", total);
    else
       sprintf( buf2, "#4There are a total of #u#2%d#n#4 visible players out of #2%d players connected.#n\n\r", nMatch,
total);
...
void do_where( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
//	sprintf( buf, "#1Area:#2%d#n\n\r" , ch->in_room->area );
//                send_to_char( buf, ch );  
	send_to_char( "#4Players near you:#n\n\r", ch );
	found = FALSE;
	for ( d = descriptor_list; d != NULL; d = d->next )
	{
	    if ( d->connected == CON_PLAYING
...
void do_email( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "Email Clear to clear your email.\r\n", ch);
	return;
    }
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( !IS_SET(ch->deaf, CHANNEL_AUCTION)
        ...
        send_to_char( !IS_SET(ch->deaf, CHANNEL_TAG)
            ? " +TAG"
            : " -tag",
            ch );
    ...
    if (IS_CLASS( ch, CLASS_ALIEN) || IS_IMMORTAL(ch))
    {
        send_to_char( !IS_SET(ch->deaf, CHANNEL_SPACE)
        ? " +SPACE"
        : " -space",
        ch );
    }

        if (IS_CLASS( ch, CLASS_REVNANT) || IS_IMMORTAL(ch))
        {
...
if (IS_CLASS( ch, CLASS_HYDRA) || IS_IMMORTAL(ch))
        {
            send_to_char( !IS_SET(ch->deaf, CHANNEL_HYDRA)
                ? " +ROAR"
                : " -roar",
                ch );
        }
	...
	if (IS_CLASS(ch, CLASS_DROWNEW) || IS_IMMORTAL(ch)) {
	send_to_char(!IS_SET(ch->deaf, CHANNEL_WEBTALK)
		? " +WEBTALK"
		: " -webtalk",
		ch ); }
	...
	else
	{
        ...
        else if ( !str_cmp( arg+1, "space"     ) ) bit = CHANNEL_SPACE;
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(  IS_SET(ch->act, PLR_BLANK)
        ...
        send_to_char(  IS_SET(ch->act, XKAFF_NOBEEP)
            ? "[+BEEPS    ] Beeps off.\n\r"
            : "[-beeps    ] Beeps on.\n\r"
            , ch );
	...
	else
	{
	    ...
	    return;
	     ...
	     if ( !str_cmp( arg+1, "nobeep"   ) ) bit = XKAFF_NOBEEP;
...
void obj_score( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    case ITEM_PTOKEN:
        sprintf( buf, "#GP#glanet #LT#loken #3V#oalue#1: #5%d#1.#n\n\r", obj->value[0] );
        send_to_char( buf, ch );
        break;
...
void do_finger( CHAR_DATA *ch, char *argument )   
{
    ...
    {
        send_to_char( "#7Thats an illegal name.\n\r", ch );
        return;
    }
 
    if ( !char_exists(TRUE,argument) )
    {
        send_to_char( "#7That player doesn't exist.\n\r", ch );
        return;
    }
...
{
send_to_char("#5-------------------------------------------------------------------------\n\r",ch);
        send_to_char("#8#1L#5ast connected ",ch);
}
        buf=fread_string( fp );
     if (IS_IMMORTAL(ch))
        send_to_char("at ",ch);
...
send_to_char("#5-------------------------------------------------------------------------\n\r",ch);
        ...
        num=fread_number( fp );
        send_to_char("#1S#5ex: ",ch);
        buf=fread_string( fp );
        if (num == SEX_MALE) send_to_char("#3Male. ",ch);
        else if (num == SEX_FEMALE)
        {
            send_to_char("#3Female. ",ch);
            if (IS_SET(extra, EXTRA_PREGNANT)) other_age(ch,extra,TRUE,buf);
        }
        else send_to_char("#1N#5one.#8 ",ch);
        buf=fread_string( fp );
        ...
        {
            default:
                send_to_char("#1T#5hey are mortal, ",ch);
                break;
            case LEVEL_AVATAR: 
            ...
            {
                if (num2 < 1)
                    sprintf(status, "#1T#5hey are an Avatar with #2%d#5 status, ",
num2);
                else if (num2 < 5)
                    sprintf(status, "#1T#5hey are an Immortal with #2%d#5 status,
", num2);
                else if (num2 < 10)   
                    sprintf(status, "#1T#5hey are a Godling with #2%d#5 status, ",
num2);
                else if (num2 < 15)
                    sprintf(status, "#1T#5hey are a Demigod with #2%d#5 status, ",
num2);
                else if (num2 < 20)
                    sprintf(status, "#1T#5hey are a Lesser God with #2%d#5 status,
", num2);
                else if (num2 < 25)
                    sprintf(status, "#1T#5hey are a Greater God with #2%d#5
status, ", num2);
                else if (num2 < 30)
                    sprintf(status, "#1T#5hey are a Supreme God with #2%d#5
status, ", num2);
                else if (num2 < 40)
                    sprintf(status, "#1T#5hey are an Ultimate God with #2%d#5
status, ", num2);
                else if (num2 < 50)
                    sprintf(status, "#1T#5hey are an Elder God with #2%d#5 status,
", num2);
                else
                    sprintf(status, "#1T#5hey are Omnipotent with #2%d#5 status,
", num2);
            ...
            case LEVEL_BUILDER:
                send_to_char("#6--=(#5They are a #7Builder#6)=--, ",ch);
                break;
            case LEVEL_QUESTMAKER: 
                send_to_char("#6--=(#5They are a #7Quest Maker#6)=--, ",ch);
                break;
            case LEVEL_ENFORCER:
                send_to_char("#6--=(#5They are an #7Enforcer#6)=--, ",ch);
                break;
            case LEVEL_JUDGE:
                send_to_char("#6--=(#5They are a #7Judge#6)=--, ",ch);
                break;
            case LEVEL_HIGHJUDGE:
                send_to_char("#6--=(#5They are a #7High Judge#6)=--, ",ch);
                break;
            case LEVEL_IMPLEMENTOR:
                send_to_char("#6--=(#5They are an #7Implementor#6)=--, ",ch);
                break;
        }
        num=fread_number( fp );
        if (num > 0) num2 = (2*(num / 7200)); else num2 = 0;
        sprintf(buf2,"and have been playing for #3%d #8hours.\n\r",num2);
        send_to_char(buf2,ch);
        buf=fread_string( fp ); 
        if (strlen(buf) > 2)
        {
            if (IS_SET(extra, EXTRA_MARRIED))
                sprintf(buf2,"#1T#5hey are married to #2%s.\n\r",buf);
            else
                sprintf(buf2,"#1T#5hey are engaged to #2%s.\n\r",buf);
            send_to_char(buf2,ch);
        }
        num=fread_number( fp );
        num2=fread_number( fp );
        sprintf(buf2,"#1P#5layer kills: #2%d#5, #1P#5layer Deaths:#2%d#5.\n\r",num,num2);
        send_to_char(buf2,ch);
        num=fread_number( fp );
        num2=fread_number( fp );
        sprintf(buf2,"#1M#5ob kills: #2%d#5, #1M#5ob Deaths:#2%d#5.\n\r",num,num2); 
        send_to_char(buf2,ch);
