...
void load_artifacts ( void )
{
   ...
   //Amulet to king.
        victim = create_mobile( get_mob_index( 8103 ) );
        char_to_room(victim, get_room_index(8120));
        obj = create_object( get_obj_index( 29504 ), 25 );
        obj_to_char(obj,victim);
        obj = create_object( get_obj_index( 8125 ), 25 );
        obj_to_char(obj,victim);
        obj = create_object( get_obj_index( 8120 ), 25 );
        obj_to_char(obj,victim);
	do_wear(victim,"all");
   //Sword & tooth to Zona
        victim = create_mobile( get_mob_index( 3001 ) );
        char_to_room(victim, get_room_index(3001));
        obj = create_object( get_obj_index( 29505 ), 25 );
        obj_to_char(obj,victim);
        obj = create_object( get_obj_index( 29506 ), 25 );
        obj_to_char(obj,victim);
        do_wear(victim,"sword");
    return;
}
...
//    pArea->reset_last	= NULL;
    pArea->filename	= fread_string( fp ); //olc
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
    ...
    *obj		= obj_zero;
    ...
    if ((obj->pIndexData->vnum >= 29500 && obj->pIndexData->vnum <= 29599)
        || ( obj->pIndexData->vnum >=  10501 && obj->pIndexData->vnum <= 10750 ))
    {
    ...
    else if ( ( obj->pIndexData->vnum >=  10751 && obj->pIndexData->vnum <= 10999 ))
    {
        SET_BIT(obj->quest, QUEST_ALIEN);
        obj->condition  = 100;
        obj->toughness  = 100;
        obj->resistance = 1;
        obj->level      = 60;
    ...
    else if ( ( obj->pIndexData->vnum >=  100701 && obj->pIndexData->vnum <= 100800 ))
    {
        ...
        || ( obj->pIndexData->vnum >=  10000 && obj->pIndexData->vnum <= 10500 )
...
void free_char( CHAR_DATA *ch )
{
    ...
    if ( ch->xkilla != NULL )
    {
        ch->xkilla->next = xkilla_free;
        xkilla_free      = ch->xkilla;
    }
