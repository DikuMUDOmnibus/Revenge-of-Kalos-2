...
void do_claws( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char("You have not Upgraded your Combat Abilities to the First Level.\n\r",ch);
	    return;
	}
	if (ch->move < 500)
	{
    ...
    else if (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_COMBAT] > 0) 
    {
	send_to_char("Your hands feel all soft and squishy now.\n\r",ch);
	act("$n's hands turn back to normal.", ch, NULL, NULL, TO_ROOM);
    }
	else
	{
	    send_to_char("Your talons slide back into your fingers.\n\r",ch);
	    act("$ns talons slide back into $s fingers.",ch, NULL, NULL, TO_ROOM);
	}
    ...
    {
	send_to_char("Your hands turn metallic.\n\r",ch);
	act("$n's hands turn metallic and shiny.", ch, NULL, NULL, TO_ROOM);
    ch->move -= 500;
    }
    else
    {
...
void do_clanname( CHAR_DATA *ch, char *argument )
{
    ...
    if ( strlen(argument) < 0 || strlen(argument) > 100 )
    {
	send_to_char( "Clan name should be between 0 and 100 letters long.\n\r", ch );
...
void do_scry( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_PPOWER(ch, PALADIN_SCRY))
	 { 
      ...
      else if (IS_CLASS(ch, CLASS_HYDRA) && ch->pcdata->powers[DISC_HYDRA_POWER] < 2 ) {
          send_to_char( "You must get the aware power before you can scry.\n\r", ch);
	    return;
          }
    ...
    else if (IS_CLASS(ch, CLASS_REVNANT) && ch->pcdata->powers[PMONK] < 2 )
    {
       ...
       return;
            ...
            && !IS_CLASS(ch, CLASS_REVNANT) && !IS_CLASS(ch, CLASS_HYDRA))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }

	if (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_OPTICAL] < 1 )
	{
	  send_to_char("You have not Upgraded your Optics to the First Level.\n\r", ch);
	  return;
	}
...
void do_readaura( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_OPTICAL] < 2 )
	{
	  send_to_char("You have not Upgraded your Optics to level Two.\n\r", ch);
	  return;
	}
	if (IS_CLASS(ch, CLASS_NINJA) && ch->move < 600 )
	{
    ...
    if (!IS_VAMPAFF(ch,VAM_AUSPEX) && !IS_ITEMAFF(ch, ITEMA_VISION) &&
        !IS_CLASS(ch, CLASS_NINJA) && !IS_CLASS(ch, CLASS_REVNANT) )
    {
	send_to_char("You are not trained in the Auspex discipline.\n\r",ch);
	return;
    }
...
(IS_CLASS(victim, CLASS_DROWNEW) &&
IS_SET(victim->xkilla->drows, DROW_OFFENSE))) && !IS_ITEMAFF(ch, ITEMA_VISION))
    {
...
void do_shield( CHAR_DATA *ch, char *argument )
{
	...
	!IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch,
CLASS_REVNANT))
    {
...
void do_regenerate( CHAR_DATA *ch, char *argument )
{
    ...
    {
	min += 15 - ch->pcdata->stats[UNI_GEN];
...
void do_upkeep( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("You are surrounded in a #1Lava Shield\n\r",ch);
	else send_to_char("You have the ability to surrouned yourself in a #1LavaShield #8though you are not.\n\r",ch);
	}
...
void do_favour( CHAR_DATA *ch, char *argument )
{
	...
	( IS_SET(ch->special,SPC_PRINCE) || ch->pcdata->stats[UNI_GEN] != 2) )
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
void do_rage( CHAR_DATA *ch, char *argument )
{
	...
	ch->pcdata->stats[UNI_RAGE] += number_range(15,25);
...
void do_totems( CHAR_DATA *ch, char *argument )
{
	...
	else if (!str_cmp(arg1,"boar"))
	{
 	...
 	else if (!str_cmp(arg1, "gaia"))
 	{
		...
		send_to_char("Lunarportal: You can traverse moonbeams to almost any locale.\n\r",ch);
	    return;
	}
	...
	sprintf(buf,"Totems: Bear (%d/4), Lynx (%d/4), Boar (%d/4), Owl (%d/4), Spider (%d/3), Wolf (%d/4),\n\r        Hawk (%d/3), Mantis (%d/4).\n\r",
	...
	else if (!str_cmp(arg1,"gaia"   )) {improve = WPOWER_GAIA;   max=4;}
...
/*it only gives 3/4 of the stats for some reason..*/
        ch->damroll += 525;
        ch->hitroll += 525;
        ch->armor -= 375;
