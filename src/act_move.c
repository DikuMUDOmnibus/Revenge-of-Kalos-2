...
char *	const	dir_name	[]		=
{
    "#1north", "#2east", "#3south", "#4west", "#5up", "#6down"
};
...
void move_char( CHAR_DATA *ch, int door )
{
	...
	(!IS_NPC(ch) && (!IS_CLASS(ch, CLASS_DROWNEW) ||
...
{
send_to_char("You cannot recall during the Arena!",ch);
return;
}
...
void do_train( CHAR_DATA *ch, char *argument )
{
    ...
    is_ok = TRUE;
    else if (!str_cmp(arg1,"skills") && IS_CLASS(ch,CLASS_DROWNEW)) 
	is_ok = TRUE;
    if ( arg2[0] != '\0' && is_ok )
    {
	int amount = 0;
    ...
    skills = (1+ch->xkilla->dskills)*2000000;
    if (IS_CLASS(ch, CLASS_HIGHLANDER)) max_stat = 25;
    if (IS_CLASS(ch, CLASS_DROWNEW)) max_stat = 25;
    ...
    else if ( !str_cmp( arg1, "hp") && ch->max_hit < 100000)
    {
    ...
    else if ( !str_cmp( arg1, "mana") && ch->max_mana < 100000)
    {
    ...
    else if ( !str_cmp( arg1, "move") && ch->max_move < 100000)
    {
	...
	ch->xkilla->dskills < 15)
	{
	int quiet_pointer=ch->xkilla->dskills;
	cost = skills;
	...
	if (ch->xkilla->dskills < 15 && IS_CLASS(ch, CLASS_DROWNEW))
	{ sprintf( buf, "Drow Skills - %d exp to gain in skills.\n\r", skills );
	  send_to_char( buf, ch);}
    ...
    if ( (*pAbility >= 100000) && (!str_cmp( arg1, "hp")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if ( (*pAbility >= 100000) && (!str_cmp( arg1, "mana")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if ( (*pAbility >= 100000) && (!str_cmp( arg1, "move")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
...
void add_tracks( CHAR_DATA *ch, int direction )
{
    ...
    || (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_TACTICAL] > 3) )
