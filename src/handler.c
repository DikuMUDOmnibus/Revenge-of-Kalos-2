...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    {
	sprintf(buf, "Eequip_char: already equipped (%s):(%d).", ch->name, iWear);
	bug( buf, 0);
//	bug( "Equip_char: already equipped (%d).", iWear );
    ...
    {
	act( "You are zapped by $pbecause you are not a demon and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	obj_to_room( obj, ch->in_room );
	do_autosave( ch, "" );
    ...
    {
        act( "You are zapped by $p because you are not a mage and drop it.", ch, obj, NULL, TO_CHAR );
        act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
        obj_from_char( obj );
        obj_to_room( obj, ch->in_room );
        do_autosave( ch, "" );
    ...
    {
        act( "You are zapped by $p because you are not a ninja and drop it.", ch, obj, NULL, TO_CHAR );
        act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
        obj_from_char( obj );
        obj_to_room( obj, ch->in_room );
        do_autosave( ch, "" );
    ...
    if ((IS_NPC(ch) || !IS_CLASS(ch, CLASS_DROWNEW)) && IS_SET(obj->spectype, SITEM_DROWWEAPON))
    {
	act( "You are zapped by $p because you are not a drow and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	obj_to_room( obj, ch->in_room );
	do_autosave( ch, "" );
    ...
    {
	act("You are zapped by $p because you are not a wolf and drop it.", ch, obj, NULL, TO_CHAR);
	act("$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM);
	obj_from_char(obj);
	obj_to_room(obj, ch->in_room);
	do_autosave(ch, "");
    ...
    if ((IS_NPC(ch) || !IS_CLASS(ch, CLASS_ALIEN))
        && IS_SET(obj->quest, QUEST_ALIEN))
    {
        act( "You are zapped by $p because you are not a alien and drop it.", ch, obj, NULL, TO_CHAR );
        act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
        obj_from_char( obj );
        obj_to_room( obj, ch->in_room );
        do_autosave( ch, "" );
...
/* removed to let demons wield demonic sword...
    if ((IS_NPC(ch) || !IS_IMMORTAL(ch)) 
	&&  IS_SET(obj->quest,QUEST_ALIEN)
        &&  obj->chobj == NULL)
    {
	act( "You are zapped by $p because it is a relic and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	obj_to_room( obj, ch->in_room );
	do_autosave( ch, "" );
 ...
 */
	...
	if (IS_SET(obj->quest, QUEST_ARTIFACT))
        ...
        if (IS_SET(obj->quest, QUEST_MYSTIC))
            SET_BIT(ch->itemaffect, ITEMA_MYSTIC);  
...
void unequip_char( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	if (IS_SET(obj->quest, QUEST_ARTIFACT)) 
        ...
        if (IS_SET(obj->quest, QUEST_MYSTIC))
            REMOVE_BIT(ch->itemaffect, ITEMA_MYSTIC);
...
char *item_type_name( OBJ_DATA *obj )
{
    ...
    case ITEM_PTOKEN:           return "planet token";
    }
