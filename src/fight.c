...
*/
    ...
    if (IS_CLASS(ch, CLASS_REVNANT) && number_range(1,3) == 2) {
	if (!IS_CLASS(ch, CLASS_HYDRA)) fightaction(ch, victim,
ch->cmbt[unarmed], dt, wieldtype);
        }
...
/* what sn?????
if (IS_CLASS(ch, CLASS_DEMON)  && IS_AFFECTED(ch, AFF_DEMONFIRE))
{

 act("#5Your #1Lava shield #5strikes $N.",ch,NULL,victim,TO_CHAR);
 act("#5$n #1Lava Shield #5strikes $N.",ch,NULL,victim,TO_NOTVICT);
 act("#5$n #1Fire Shield Strikes you.",ch,NULL,victim,TO_VICT);
...
*/
	...
	spell_poison( gsn_poison, (ch->level*number_range(5,10)),ch,victim );
    else if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_COMBAT] > 4)
	spell_poison( gsn_poison, (ch->level*number_range(5,10)),ch,victim );
...
int number_attacks( CHAR_DATA *ch )
{
    ...
    if ( IS_CLASS(ch, CLASS_HIGHLANDER)) count -= 1;
    if (!IS_NPC(ch))
    {
    	if ( IS_CLASS(ch, CLASS_VAMPIRE) && IS_VAMPAFF(ch, VAM_CELERITY) )
		count += 1;
if (IS_CLASS(ch, CLASS_VAMPIRE)) {
            if (ch->pcdata->rank == AGE_METHUSELAH) count += 1;
            if (ch->pcdata->rank == AGE_ANCILLA) count += 1;
            }
...
/* Ninja speed was taken out for ninja daggers were used instead - Vic */
        else if ( IS_CLASS(ch, CLASS_REVNANT) && ch->pcdata->powers[PMONK] > 4 )
		count += 1;
   	else if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_DEMON) && IS_DEMPOWER( ch, DEM_SPEED) )
		count += 1;
	else if (IS_CLASS(ch, CLASS_DROWNEW) && (IS_SET(ch->xkilla->drows, DROW_OFFENSE)))
		count += 3;
        else if (IS_ALIPOWER( ch, ALI_SPEED) && IS_CLASS(ch,CLASS_ALIEN))
		count += 2; 
...
/*S*/
    if (IS_CLASS(ch, CLASS_HYDRA)) count =
			ch->pcdata->powers[DISC_HYDRA_HEADS];
    return count;
}
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, int handtype)
{
	...
	dam *= 1.5;
		...
		dam *= 1.3;
	else if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_PALADIN))
		dam *=1.75;
    else if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_NINJA) &&
	ch->pcdata->powers[NPOWER_TACTICAL] > 1 )
	dam *= 1.2;
    else if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_DEMON) && 
		IS_DEMPOWER(ch, DEM_MIGHT) )
	dam *= 1.2;
        else if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_DRAGON) )
                dam *= 1.2;
    if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_WEREWOLF) )
    {
	if (IS_SET(victim->special, SPC_WOLFMAN)) dam *= 0.6;
...
//class with over 200 weapwon power got lowered
    if ( !IS_NPC(ch) && dt >= TYPE_HIT)
    {
        if (IS_CLASS(ch, CLASS_REVNANT)|| IS_CLASS(ch, CLASS_HIGHLANDER) ||
	    (IS_CLASS(ch, CLASS_DROWNEW) && IS_SET(ch->xkilla->drows, DROW_OFFENSE)))
	    dam = dam + (dam * ((ch->wpn[dt-1000]+1) / 200));
	else
...
*/
	...
	dam = 1;
/*S*/
    if (IS_CLASS(ch, CLASS_HYDRA)) dam = 0;
    if (!IS_NPC(ch) 
	&& ch->level < 6 
	&& ch->max_hit > 110000)
    {
	send_to_char("Report to imp of your HP!.\r\n", ch);
	send_to_char("And why you had it.\r\n", ch);
	dam = 0;
    }
...
void update_damcap( CHAR_DATA *ch, CHAR_DATA *victim )
{
	    ...
	    ch->in_room != NULL)
	{
		...
		max_dam += 100;
	    if (ch->in_room->vnum == 30000) max_dam = 5000; /*was5000Rotain*/
	}
        else if (IS_CLASS(ch, CLASS_REVNANT))  max_dam += 1200;
	else if (IS_CLASS(ch, CLASS_PALADIN)) max_dam += 2000;
	...
	max_dam += 100;
	max_dam += (ch->xkilla->dskills * 100);
            if (ch->in_room->vnum == 100701) max_dam = 30000;
        }
	else if (IS_CLASS(ch, CLASS_MAGE)) max_dam += 1000;
	...
	{
		if (ch->pcdata->stats[UNI_GEN] < 3) max_dam = 2300;
	    else if (ch->pcdata->stats[UNI_GEN] < 4) max_dam = 2300;
	    else if (ch->pcdata->stats[UNI_GEN] < 5) max_dam = 2200;
	    else                      max_dam = 2100;
	}
        else if (IS_CLASS(ch, CLASS_ALIEN))
        {
                if (ch->pcdata->stats[UNI_GEN] < 1) max_dam = 3000;
            else if (ch->pcdata->stats[UNI_GEN] < 2) max_dam = 2800;
            else if (ch->pcdata->stats[UNI_GEN] < 3) max_dam = 2600;
            else                      max_dam = 2500;
        }
	else if (IS_SET(ch->special, SPC_WOLFMAN))
	{	
	    if      (ch->pcdata->stats[UNI_GEN] < 2) max_dam = 2500;
	    else if (ch->pcdata->stats[UNI_GEN] < 3) max_dam = 2400;
	    else if (ch->pcdata->stats[UNI_GEN] < 4) max_dam = 2300;
	    else                      max_dam = 2200;
...
/*	    int wpn = ch->pcdata->powers[HPOWER_WPNSKILL];
	...
	max_dam += (ch->race * 50 );
        if (IS_ITEMAFF(ch, ITEMA_MYSTIC)) max_dam += 500;
        if (IS_ITEMAFF(victim, ITEMA_MYSTIC)) max_dam -= 500;
...
/*S*/
        ...
        {
       max_dam = 350;
       if (!IS_NPC(victim)) max_dam -= 150;
       if (ch->pcdata->powers[DISC_HYDRA_POWER] < 4) max_dam -= 100;
       if (ch->pcdata->powers[DISC_HYDRA_POWER] < 1) max_dam -= 100;
       if (IS_CLASS(victim, CLASS_ALIEN)) max_dam += 50;
         }
...
void damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if ( IS_CLASS(ch, CLASS_NINJA) && IS_SET(ch->act, PLR_WIZINVIS) )
	{
        ...
        else if ( IS_CLASS(ch, CLASS_REVNANT) && IS_SET(ch->act, PLR_WIZINVIS) )
        {
            if (!can_see(victim,ch))
            {
            dam *= 1.2;
        ...
        if ( IS_AFFECTED(victim, AFF_SANCTUARY) && dam > 1 ) {
            dam /= 2;
            if (IS_CLASS(victim, CLASS_HYDRA))
            dam *= 2;
            }
        if ( IS_CLASS(victim, CLASS_HYDRA) && dam > 1 ) {
            if (victim->pcdata->powers[DISC_HYDRA_POWER] >= 5)
            dam /= 2;
            if (IS_CLASS(victim, CLASS_HYDRA) && dam > 1 && victim->pcdata->powers[DISC_HYDRA_POWER] >= 5)
            dam /= 2;
            if (IS_CLASS(victim, CLASS_HYDRA) && IS_NPC(ch)) {
            dam /= 1.6; /*THEY FUCKING SUCK agaisnt mobs without this*/
			//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
			//Who added that??? they get a max_dam of 450
			// dam *= 10000 would not matter!! --cheerios
            if (ch->level >1000) dam *=1.3;
            if (ch->level >1600) dam *=1.2;
            if (ch->level >2300) dam *=1.2;
            if (ch->level >5000) dam *=1.4;
            }
            }

        if (IS_CLASS(victim, CLASS_ALIEN) && IS_ALIPOWER(victim, ALI_TOUGH) && dam > 1 )
	 {
	    dam /= 2;
         }
...
/*S*/
    if ( IS_CLASS(ch, CLASS_HYDRA) ) {
       dam = 350;
       if (!IS_NPC(victim)) dam -= 150;
       if (ch->pcdata->powers[DISC_HYDRA_POWER] < 4) dam -= 100;
       if (ch->pcdata->powers[DISC_HYDRA_POWER] < 1) dam -= 100;
       if (IS_CLASS(victim, CLASS_ALIEN)) dam += 50;
            }
        dam_message( ch, victim, dam, dt );
    }
    if (dt == gsn_pyro) dam = 1000;
    if (dt == gsn_circle) dam = 2000;
...
/*    default:
        ...
        if (IS_NPC(victim) && !IS_NPC(ch))
	{
        ...
        if (IS_CLASS(ch, CLASS_ALIEN) && IS_NPC(victim) && !IS_SET(victim->act, ACT_NOEXP ))
                ch->pcdata->stats[ALIEN_PLANET] += victim->level*(number_range(200, 150)/ 75);
		...
		if ((vnum = victim->pIndexData->vnum) > 29000)
		{
		    ...
		    {
		    default:
			ch->pcdata->stats[DEMON_CURRENT] +=victim->level*(number_range( 170, 250 )/100);
			ch->pcdata->stats[DEMON_TOTAL]   +=victim->level*(number_range( 170, 250 )/100);
			break;
		    case 29600:
		    case 30001:
		    case 30006:
	...
	if ( IS_SET(ch->act, PLR_AUTOLOOT) )
	    ...
	    do_look( ch, "in corpse" );
            ...
            if ( !IS_NPC(ch) && IS_SET(ch->xkilla->plr, PLR_AUTOCONSUME) )
                do_consume( ch, "" );
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if (IS_CLASS(victim, CLASS_HYDRA) && IS_NPC(ch))
    { obj=NULL;claws=TRUE;}
    else if ( IS_CLASS(victim, CLASS_REVNANT ) &&
		victim->pcdata->powers[PMONK] >= 5 )
    {
    obj = NULL;
    hands = TRUE;
    }
    ...
    if ((claws) && !IS_CLASS(ch, CLASS_HYDRA)) 
    {
    ...
    if (hands) 
    {
	if ( victim->pcdata->powers[PMONK] >= 5 )
	    chance += (char_hitroll(victim) * 0.1);
	else
	    chance += (char_hitroll(victim) * 0.075);
    }
    ...
    if (!IS_NPC(ch))
    {
       ...
       if (ch->pcdata->powers[NPOWER_COMBAT] > 4)
	    ...
	    chance -= 15;
        else if (IS_CLASS(ch, CLASS_ALIEN) && IS_ALIPOWER(ch, ALI_SPEED) )
            chance -= 35;
		...
		chance -=15;
	else if (IS_CLASS(ch, CLASS_DROWNEW) && IS_DPOWER(ch, DROW_OFFENSE))
		chance -= 25;
	else if (IS_CLASS(ch, CLASS_WEREWOLF) &&
	    ch->pcdata->powers[WPOWER_MANTIS] < 5 )
	    chance -= (ch->pcdata->powers[WPOWER_MANTIS] * 5);
        else if (IS_CLASS(ch, CLASS_REVNANT) && ch->pcdata->powers[PMONK] > 3 )
	{
...
//dam rigged monks
	    ...
	    else chance -= (ch->pcdata->powers[PMONK] * 6 );
	}
/* enoguth with 1k slice already!!!
	else if (IS_CLASS(ch, CLASS_HIGHLANDER) && (get_eq_char(ch, WEAR_WIELD) != NULL))
		chance -= ch->race*2/3;
*/
    }
	...
	if (IS_VAMPAFF(victim, VAM_CELERITY) && 
	    ...
	    chance += 20;
	else if (IS_CLASS(victim, CLASS_NINJA) && victim->pcdata->powers[NPOWER_TACTICAL] > 2 )
	    chance += 20;	
	else if (IS_CLASS(victim, CLASS_DEMON) && IS_DEMPOWER( victim, DEM_SPEED) )
	    chance += 15;
        else if (IS_CLASS(victim, CLASS_ALIEN) && IS_ALIPOWER( victim, ALI_SPEED) )
            chance += 35;
	else if (IS_CLASS(victim, CLASS_MAGE))
	    chance += 15;
	else if (IS_CLASS(victim, CLASS_PALADIN))
	chance += 40;
        else if (IS_CLASS(victim, CLASS_DROWNEW) && IS_DPOWER(victim, DROW_OFFENSE))     
		chance += 25;
/* enough with 1k slice!!
...
*/	
    ...
    if (IS_NPC(ch) && IS_CLASS(victim, CLASS_HYDRA)) {
       if (victim->pcdata->powers[DISC_HYDRA_TWIRL] >0 ) chance += 4;
       if (victim->pcdata->powers[DISC_HYDRA_TWIRL] >1 ) chance += 4;
       if (victim->pcdata->powers[DISC_HYDRA_TWIRL] >2 ) chance += 4;
       if (victim->pcdata->powers[DISC_HYDRA_TWIRL] >3 ) chance += 4;
       if (victim->pcdata->powers[DISC_HYDRA_TWIRL] >4 ) chance += 4;
       if (victim->pcdata->powers[DISC_HYDRA_TWIRL] >5 ) chance += 4;
       if (victim->pcdata->powers[DISC_HYDRA_TWIRL] >6 ) chance += 8;
       if (ch->level >1000) chance -=5;
       if (ch->level >1500) chance -=4;
       if (ch->level >2000) chance -=5;
       if (ch->level >3000) chance -=6;
       if (ch->level >4500) chance -=9;
       }
/*S*/
/* funny -cheerios
a 20 headed hydra hits 3 times without this - smoo
*/
    if ( IS_CLASS(ch, CLASS_HYDRA)) 
    return FALSE;
	...
	if (IS_NPC(victim) || !IS_SET(victim->act, PLR_BRIEF))
       act("$n's weak blow does nothing to your strong body.", ch, NULL,
victim, TO_VICT);
	if (IS_NPC(ch) || !IS_SET(ch->act, PLR_BRIEF))
       act("Your weak blow does nothing to $N's strong body!", ch, NULL,
victim, TO_CHAR);
        }
        else {
	if (IS_NPC(victim) || !IS_SET(victim->act, PLR_BRIEF))
...
/*S*/
/* funny
cheerios, i put that in for a reason, i WILL weaken them when
I get a chance*/
    if (IS_CLASS(ch, CLASS_HYDRA))
        return FALSE;
    if ( !IS_AWAKE(victim) )
    ...
    if (!IS_NPC(ch))
    {
		...
		&& IS_SET(ch->xkilla->drows, DROW_OFFENSE))
		chance -=  25;
	else if (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_TACTICAL] > 2 )
	    chance -= 20;
	else if (IS_CLASS(ch, CLASS_WEREWOLF) &&
	    ch->pcdata->powers[WPOWER_MANTIS] < 5 )
	    chance -= (ch->pcdata->powers[WPOWER_MANTIS] * 5);
        else if (IS_CLASS(ch, CLASS_REVNANT) &&
...
*/ 
    ...
    if (!IS_NPC(victim))
    {
	 if (IS_VAMPAFF(victim, VAM_CELERITY) && 
	    ...
	    chance += 20;	 
	else if (IS_CLASS(victim, CLASS_DROWNEW) &&
		IS_SET(victim->xkilla->drows, DROW_OFFENSE))
		chance += 25;
	else if (IS_CLASS(victim, CLASS_MAGE))
		chance += 15;
	else if (IS_CLASS(victim, CLASS_PALADIN))
...
if (!IS_NPC(victim) && IS_SET(victim->extra, EXTRA_STANCE) &&
		victim->stance[0] < 0)
    {
      if ( victim->pcdata->autostance == STANCE_VIPER )
        do_stance(victim,"viper");
      else if ( victim->pcdata->autostance == STANCE_CRANE )
        do_stance(victim,"crane");
      else if ( victim->pcdata->autostance == STANCE_CRAB )
        do_stance(victim,"crab");
      else if ( victim->pcdata->autostance == STANCE_MONGOOSE )
        do_stance(victim,"mongoose");
      else if ( victim->pcdata->autostance == STANCE_BULL )
        do_stance(victim,"bull");
      else if ( victim->pcdata->autostance == STANCE_MANTIS )
        do_stance(victim,"mantis");
      else if ( victim->pcdata->autostance == STANCE_DRAGON )
        do_stance(victim,"dragon");
      else if ( victim->pcdata->autostance == STANCE_TIGER )
        do_stance(victim,"tiger");
      else if ( victim->pcdata->autostance == STANCE_MONKEY )
        do_stance(victim,"monkey");
      else if ( victim->pcdata->autostance == STANCE_SWALLOW )
        do_stance(victim,"swallow");
    }

    if (!IS_NPC(ch) && IS_SET(ch->extra, EXTRA_STANCE) && ch->stance[0] < 0)
    {
      if ( ch->pcdata->autostance == STANCE_VIPER )
        do_stance(ch,"viper");
      else if ( ch->pcdata->autostance == STANCE_CRANE )
        do_stance(ch,"crane");
      else if ( ch->pcdata->autostance == STANCE_CRAB )
        do_stance(ch,"crab");
      else if ( ch->pcdata->autostance == STANCE_MONGOOSE )
        do_stance(ch,"mongoose");
      else if ( ch->pcdata->autostance == STANCE_BULL )
        do_stance(ch,"bull");
      else if ( ch->pcdata->autostance == STANCE_MANTIS )
        do_stance(ch,"mantis");
      else if ( ch->pcdata->autostance == STANCE_DRAGON )
        do_stance(ch,"dragon");
      else if ( ch->pcdata->autostance == STANCE_TIGER )
        do_stance(ch,"tiger");
      else if ( ch->pcdata->autostance == STANCE_MONKEY )
        do_stance(ch,"monkey");
      else if ( ch->pcdata->autostance == STANCE_SWALLOW )
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( !is_same_group( gch, ch ) )
	    continue;
                     ...
                     {
                        case  1:
        xp = xp_compute( gch, victim ) / members * 10;
        sprintf( buf, "You receive %d experience points.\n\r", xp );
        send_to_char( buf, gch );
        sprintf( buf, "You receive bonus xp due to #2Xkilla#n.\n\r" );
        send_to_char( buf, gch );
                    break;
                        case  2:
...
//	send_to_char( buf, gch );
	if (dxp_info.dxp == 1)
	{
	        sprintf( buf, "You receive %d experience points due to bonus xp.\n\r", xp );
        send_to_char( buf, gch );
	xp = xp * 2;
	}
	else if (dxp_info.dxp == 0)
	xp = xp * 1;
	else
	xp = xp * 1;
...
/*  xp    -= xp * URANGE( -2, extra, 8 ) / 8;
    ...
    xp    -= xp * number_range( -3, 3 ) / 8;

    xp     = number_range( xp * 5 / 4, xp * 6 / 4 );
    xp     = UMAX( 0, xp );
    xp     = (xp * (victim_level+1) * 1.5);
    xp     = xp / 5; /* Put in cause players compaling to much exp :P */
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    {
	"#1hit",
	"#2slice",  "#7stab",  "#6slash", "#4whip", "#1claw",
	"#3blast",  "#6pound", "#5crush", "#3grep", "#2bite",
	"#4pierce", "#5suck"
    };
    static char * const attack_table2[] =
    {
	"#1hits",
	"#2slices",  "#7stabs",  "#6slashes", "#3whips", "#2claws",
	"#3blasts",  "#6pounds", "#5crushes", "#4greps", "#1bites",
	"#4pierces", "#5sucks"
    };
    ...
    else if ( dam < 2500 ) { vs = " insanely hard";vp = " insanely hard";}
    else                   { vs = " with UNGODLY FORCE";vp = " with UNGODLY FORCE";}
	...
	(IS_NPC(victim) && IS_SET(victim->act, ACT_NOPARTS)) )
    {
		...
		victim->hit = victim->hit+number_range(1,(dam-recover));
            else if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_REVNANT) &&
		...
		victim->hit = victim->hit+number_range(1,(dam-recover));
            else if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_ALIEN)
                && (dam-recover) > 0 && IS_ALIPOWER( victim, ALI_TOUGH))
                victim->hit = victim->hit+number_range(1,(dam-recover));
	else if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_PALADIN) &&
		(dam-recover) > 0)
		victim->hit = victim->hit +number_range(1,((dam-recover)));
	    else if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_NINJA) &&
		victim->pcdata->powers[NPOWER_TACTICAL] > 0 && (dam-recover) > 0)
		victim->hit = victim->hit+number_range(1,(dam-recover));
	    else if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_DROWNEW) &&
		IS_SET(victim->xkilla->drows, DROW_OFFENSE) &&
		(dam-recover) >0)
		victim->hit = victim->hit+number_range(1,(dam-recover));
	    ...
	    {
          int legsman = 0;
          legsman = number_range(1, 6);
             if (legsman == 1) {
               sprintf( buf1, "$n hits $N.", dam); 
               sprintf( buf2, "You hit $N. [%d]", dam);
               sprintf( buf3, "$n hits you. [%d]", dam);
                 }
               else if (legsman == 2) {
               sprintf( buf1, "$n completely #sDESTROYS#n $N!"); 
               sprintf( buf2, "You completely #sDESTROY#n $N! [%d]", dam);
               sprintf( buf3, "$n completely #sDESTROYS#n you! [%d]",
dam);
                  }
               else if (legsman == 3)  {
...
void trip( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if (IS_SET(victim->special, SPC_CHAMPION) && IS_DEMAFF(victim,DEM_UNFOLDED))
	    return;
	if (IS_CLASS(victim, CLASS_DROWNEW) &&
IS_SET(victim->xkilla->drows, DROW_OFFENSE)) return;
    }
    if ( victim->wait == 0)
    {
...
*/
	...
	return;
    		   ...
    		   ch->pcdata->powers[NPOWER_COMBAT] > 1 
...
void do_flee( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if (IS_CLASS(ch,CLASS_VAMPIRE) && number_percent() <= (ch->pcdata->stats[UNI_RAGE]/2))
	{
...
if (is_safe(ch, victim)) return;
	...
	ch->pcdata->powers[NPOWER_TACTICAL] > 2 )
	dam *= 1.1;
...
void do_berserk( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_CLASS(ch, CLASS_HYDRA)) {
       send_to_char( "Hydra's can't berserk!\n\r", ch);
       return;
       }
...
!IS_SET(victim->special,SPC_CHAMPION))
	...
	else
	{
        ...
        {
        ch->pcdata->stats[ALIEN_PLANET] += victim->race* 1000 + 1000;
        }
...
/* on the 6th line, no argument for $N.
act( "             _______           ",  ch, NULL, NULL, TO_NOTVICT );  
act( "           .'=======`.         ",  ch, NULL, NULL, TO_NOTVICT );
act( "           | _______ |         ",  ch, NULL, NULL, TO_NOTVICT );
act( "  __|_|__  | __RIP__ | __|_|__ ",  ch, NULL, NULL, TO_NOTVICT );
act( "           |         |         ",  ch, NULL, NULL, TO_NOTVICT );
act( "           |$N       |	       ",  ch, NULL, NULL, TO_NOTVICT );
act( "    ....   ._________.  ....   ",  ch, NULL, NULL, TO_NOTVICT );
*/
...
void do_tear( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_REVNANT))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
void do_moongate(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_CLASS(ch, CLASS_WEREWOLF))
   {
    ...
    if (ch->pcdata->powers[WPOWER_GAIA] < 4)
    {
      send_to_char("You need to obtain level 4 Gaia to use Lunar Portal.\n\r",ch);
      return;
    }
...
//    ||   IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL)
  ...
  if (ch->mana < 500) 
        {
                send_to_char("You have not the required mana to open a Lunar Portal.\n\r",ch);
                return;
        }
    
        ch->mana -= 500;
        do_say(ch, "Lunar Portal Appear!\n\r");  
	send_to_char("A moonbeam rips a hole in reality right in front of you. You step into it and appear elsewhere.\n\r",ch);
        char_from_room(ch);
        char_to_room(ch, get_room_index(victim->in_room->vnum));
        do_look(ch,"auto");
        WAIT_STATE( ch, 16 );
        act( "You appear in front of $N in a beam of moonlight.", ch, NULL, victim, TO_CHAR);
    act( "$n appears in front of you in a beam of moonlight.", ch, NULL, victim, TO_VICT);
    act( "$n appears in front of $N in a beam of moonlight.", ch, NULL, victim, TO_NOTVICT);
     return;
}
