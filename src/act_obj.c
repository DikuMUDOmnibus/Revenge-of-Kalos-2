...
void do_eat( CHAR_DATA *ch, char *argument )
{
    ...
    case ITEM_PTOKEN:
        ch->pcdata->stats[ALIEN_PLANET] += obj->value[0];
        break;
...
IS_SET(obj->spectype, SITEM_VAMPIRE)) {
       act("$n is shocked by $p and drops it.", ch, obj, NULL, TO_ROOM);
       act("You are shocked by $p and drop it!", ch, obj, NULL, TO_CHAR);
       obj_from_char(obj);
       obj_to_room(obj,ch->in_room);
       return;
      }
	...
	if (!is_ok_to_wear(ch,wolf_ok,"left_hand") && !is_ok_to_wear(ch,wolf_ok,"right_hand"))
        ...
        else if (IS_CLASS(ch, CLASS_REVNANT) && IS_HERO(ch))
...
void do_wear( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_CLASS(ch, CLASS_HYDRA)) {
        send_to_char("Hydras cannot wear equipment!\n\r", ch);
        return;
    }
    if ( arg[0] == '\0' )
    {
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
        sprintf( buf, "You receive a refund of 50 primal from $p");
        act(buf, ch, obj, NULL, TO_CHAR);
        ch->practice += 80;
    }
    extract_obj( obj );
...
{
send_to_char("You can't activate, your in the arena!",ch);
return;
}
...
/*poor hydras can't activate with this in :(
	if (CAN_WEAR(obj, ITEM_TAKE))
	{
...
bool is_ok_to_wear( CHAR_DATA *ch, bool wolf_ok, char *argument )
{
	...
	/* monks cant use weap when avatar */
        if (IS_CLASS(ch, CLASS_REVNANT) && IS_HERO(ch)) return FALSE;
...
void quest_object( CHAR_DATA *ch, OBJ_DATA *obj )
{
    static const sh_int quest_selection[] = 
    {
	 ...
	 5005, 5011, 5012, 5013, 2902, 1352, 2348, 2361, 5011, 102,
	 ...
	 2331, 2382, 8008, 8005, 5300, 5302, 5309, 5310, 5311, 4000,
