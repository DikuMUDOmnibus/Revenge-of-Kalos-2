...
{
send_to_char("You cannot use goto in tag!",ch);
return;
}
...
void do_pload( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp("order",arg)) {
	send_to_char(" You failed.\n\r", ch );
	return;}
      
        if (!str_cmp("defiance",arg)) {
...
void do_pload( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp(argument,"Sage")) {
	send_to_char("You failed.\n\r", ch );
	return;}

        if (!str_cmp(argument,"Defiance")) {
        send_to_char("You failed.\n\r", ch ); 
        return;}
		...
		else if (!IS_JUDGE(ch))
		{
		    ...
		    return;
		     ...
		     if ( !str_cmp( arg3, "mortal"     ) ) value = 2;
                ...
                else if ( !str_cmp( arg3, "revnant"       ) ) value = 6;
...
Apprentice, Mage, Archmage, Revnant, Ninja, Builder, Questmaker, Enforcer,\n\rJudge, or
Highjudge.\n\r", ch );
		    return;
		}
	
		if (!str_cmp("sage",arg1)) {
        	send_to_char(" You failed.\n\r", ch );
        	return;
        }

                if (!str_cmp("defiance",arg1)) {
                send_to_char(" You Failed.\n\r", ch );                               
                return;
        }
		
		if (!str_cmp("order",arg1)) {
        	send_to_char(" You failed.\n\r", ch );
        	return;
        }
	...
	if ( value > 5000000000 )
	{
	    send_to_char( "No more than 5000000000 possible.\n\r", ch );
	    return;
	}
...
void do_oset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  relic artifact mystic \n\r",			ch );
	return;
    }
	...
	if (obj->item_type == ITEM_WEAPON && !IS_JUDGE(ch)) {
		...
		return;}
        else if (obj->item_type == ITEM_PTOKEN) {
                send_to_char("You cannot change planet tokens value with oset.\n\r", ch);
                return;}
...
Hands, Arms, About, Waist, Hold, Face.\n\r",ch); return;}
    ...
    if ( !str_cmp( arg2, "mystic" ) )
    {
        if (!IS_JUDGE(ch))
        {
            send_to_char("You are not authorised to make an item an mystic.\n\r", ch);
            return;
        }
      if (!IS_SET(obj->quest, QUEST_MYSTIC))
      {
        SET_BIT(obj->quest, QUEST_MYSTIC);
          send_to_char("You make the item a mystic!\n\r",ch);
        return;
      }
      if (IS_SET(obj->quest, QUEST_MYSTIC))
      {
        REMOVE_BIT(obj->quest, QUEST_MYSTIC);
          send_to_char("The item is no longer an mystic.\n\r",ch);
        return;
      }
        else
      {
        send_to_char("Error....in setting QUEST_MYSTIC.\n\r",ch);
        return;
      }
    }
...
#define EXE_FILE	  "../src/RoK"


/*  Copyover - Original idea: Fusion of MUD++
		Copyover from their mud transfered
to a godwars database by XKilla
 */
void do_hotboot (CHAR_DATA *ch, char * argument)
{
	...
	if (!fp)
	{
		...
		logf ("Could not write to hotboot file: %s",COPYOVER_FILE);
	...
	for (d = descriptor_list; d ; d = d_next)
	{
		...
		{
			fprintf (fp, "%d %s %s %d\n", d->descriptor, och->name, d->host,  (d->out_compress) ? 1 : 0);
		    //    if (d->out_compress)
                //	    compressEnd(d);
	...
	execl (EXE_FILE, "RoK", buf, "hotboot", buf2, (char *) NULL);

	/* Failed - sucessful exec will not return */
	
	perror ("do_hotboot: execl");
	send_to_char ("Hot boot FAILED!\n\r",ch);
...
void copyover_recover ()
{
	...
	char name [100];
	char host[MSL];
	int desc, yes_compress;
	bool fOld;
	
	logf ("Hotboot recovery initiated");
	...
	if (!fp) /* there are some descriptors open which will hang forever then ? */
	{
		...
		logf ("Hotboot file not found. Exitting.\n\r");
		exit (1);
	}
...
void do_artifact( CHAR_DATA *ch, char *argument )
{
    ...
    found = FALSE;
	send_to_char("The Artifacts found in the land of kalos are: \n\r",ch);
...
void do_clearstats( CHAR_DATA *ch, char *argument )
{
    ...
    save_char_obj( ch );
    send_to_char("Your stats have been cleared.  Please rewear your equipment.\n\r",ch);
/*added in because of a fucking zuloform bug..*/
    clear_stats(ch);
    return;
}
...
if ( !IS_NPC(ch) )
    ...
    if ( IS_NPC(ch) || (ch->pcdata->stats[DEMON_CURRENT] < 1 && !IS_IMMORTAL(ch)) )
 {
    ...
    else
    {
        ...
        else if ( value > ch->pcdata->stats[DEMON_CURRENT] && !IS_IMMORTAL(ch) )
        {
            sprintf(buf,"You only have %d demon points left to put into
tokens.\n\r",ch->pcdata->stats[DEMON_CURRENT]);
    ...
    if (victim != NULL && victim != ch)
    {
        act( "You pull $p out of $N's pocket.", ch, obj, victim, TO_CHAR );
        act( "$n pulls $p out of $s pocket.", ch, obj, victim, TO_NOTVICT );
        act( "$n pulls $p out of $N's pocket.", ch, obj, victim, TO_VICT );
    }
    else
    {
