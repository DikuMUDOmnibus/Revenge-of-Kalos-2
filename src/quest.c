...
/* And, of course, you will need to change the following lines for YOUR
   ...
   build up quest points :> Make the item worth their while. */
    ...
    {
        bug( "Missing object in Quest.", 0 );
     return;
    }

    if (!strcmp(arg1, "list"))
    {
        act3("$n asks $N for a list of quest items.",ch,NULL,questman,TO_ROOM); 
	act3("You ask $N for a list of quest items.",ch,NULL,questman,TO_CHAR);
...
QUEST_VALUE5, obj5->short_descr);
        ...
        else if (is_name(arg2, "6"))
	{
		...
		ch->questpoints -= 500;
	        ch->practice += 30;
    	        act3("$N gives 30 practices to $n.", ch, NULL, questman, 
			TO_ROOM );
    	        act3("$N gives you 30 practices.",   ch, NULL, questman, 
			TO_CHAR );
	        return;
	    }
	    else
        ...
        else if (is_name(arg2, "7"))
	{
		...
		ch->questpoints -= 500;
                ch->gold += 10000000;
                act3("$N gives 10,000,000 gold pieces to $n.", ch, NULL, 
			questman, TO_ROOM );
                act3("$N hands you a pouch of gold pieces.",   ch, NULL, 
			questman, TO_CHAR );
	        return;
	    }
	    else
	...
	if (obj != NULL)
	{
            act3("$N gives something to $n.", ch, obj, questman, TO_ROOM );
            act3("$N gives you your reward.",   ch, obj, questman, TO_CHAR );
	    obj_to_char(obj, ch);
	}
	return;
    }
    ...
    {
        act3("$n asks $N for a quest.", ch, NULL, questman, TO_ROOM); 
	act3("You ask $N for a quest.",ch, NULL, questman, TO_CHAR);
        if (IS_SET(ch->act, PLR_QUESTOR))
	{
    ...
    {
        act3("$n informs $N $e has completed $s quest.", ch, NULL, questman, 
		TO_ROOM); 
        act3("You inform $N you have completed $s quest.",ch, NULL, 
		questman, TO_CHAR);
        ...
        if (IS_SET(ch->act, PLR_QUESTOR))
	{
		...
		if (obj_found == TRUE)
		{
		    ...
		    act3("You hand $p to $N.",ch, obj, questman, TO_CHAR);
		    act3("$n hands $p to $N.",ch, obj, questman, TO_ROOM);
