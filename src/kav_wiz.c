...
return;
	...
	if (!str_cmp(ch->name,"Xkilla") || !str_cmp(ch->name,"Beltaine")||
	!str_cmp(ch->name,"Cheerios")  ||  !str_cmp(ch->name,"Rico"))
	...
	ch->pcdata->security = 9;
        ...
        send_to_char("What ever you say boss!\n\r",ch);
    }
    else
	send_to_char("Don't try to relevel FUCKER!\n\r",ch);
    return;
}
...
*/
	...
	{
	    send_to_char("Negatives are not allowed.\r\n", ch);
	    return;
	}
...
/*
	...
	if (obj->item_type == ITEM_QUEST || obj->item_type == ITEM_PTOKEN || obj->item_type == ITEM_QUESTCARD)
	{
...
void do_makepreg( CHAR_DATA *ch, char *argument )
{
	...
	save_char_obj(victim1);
	save_char_obj(victim2);
	sprintf(buf,"%s and %s are pregnant!",victim1->name,victim2->name);
	do_info(ch,buf);
	return;
    }
    send_to_char("But they are not yet engaged!\n\r",ch);
