...
void do_pset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  aura dps sps status \n\r",	        ch );
	send_to_char( "\n\r",						ch );
...
/* psetting aura points is possible - Loki */
...
if ( value < 1 || value > 200000 )
{
    send_to_char( "Shatan points range is 1 to 200000.\n\r", ch );
    return;
}

if (IS_JUDGE(ch)) {
    ch->pcdata->stats[SHATAN_CURRENT] = value;
    ch->pcdata->stats[SHATAN_TOTAL] = value;
    send_to_char("Ok.\n\r",ch);}
else
    send_to_char( "Sorry, no can do...\n\r", ch );
return;
