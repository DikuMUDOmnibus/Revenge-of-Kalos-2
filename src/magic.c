...
void improve_spl( CHAR_DATA *ch, int dtype, int sn )
{
	...
	( (dice1==99 || dice1==100) || (dice2==99 || dice2==100)) )
	ch->spl[dtype] += 1;
    else return;
...
void spell_dispel_magic( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (IS_AFFECTED2(victim, AFF_MONK_CURSE)) mcurse = TRUE;
	...
	if (mcurse == TRUE)
	{
 	REMOVE_BIT(victim->affected_by2, AFF_MONK_CURSE);
...
void spell_levitation( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.bitvector = AFF_FLYING;
    affect_to_char( victim, &af );
    send_to_char( "You begin to float a couple feet of the ground.\n\r", victim );
    act( "$n begins to float a couple feet of the ground.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
/*
    if (obj->value[0] > 0 || obj->item_type != ITEM_PTOKEN )
    {
        sprintf( buf, "#GP#glanet #Lp#loint #3v#oalue#1: #6%d#1.#n\n\r", obj->value[0] );
        send_to_char( buf, ch );
    }
*/
	...
	sprintf( buf, "Weapontype is: %s\n\r",weap_type[obj->value[3]]);
		send_to_char( buf, ch);
...
void spell_poison( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    && victim->pcdata->powers[NPOWER_COMBAT] > 4 ) return;
...
void spell_soulblade( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    weapontype = (number = number_range(1, 12));

if (weapontype == 1) strcpy(wpnname,"slicer");
if (weapontype == 2) strcpy(wpnname,"stabber");
if (weapontype == 3) strcpy(wpnname,"slasher");
if (weapontype == 4) strcpy(wpnname,"whipper");
if (weapontype == 5) strcpy(wpnname,"clawer");
if (weapontype == 6) strcpy(wpnname,"blaster");
if (weapontype == 7) strcpy(wpnname,"pounder");
if (weapontype == 8) strcpy(wpnname,"crusher");
if (weapontype == 9) strcpy(wpnname,"grepper");
if (weapontype == 10) strcpy(wpnname,"biter");
if (weapontype == 11) strcpy(wpnname,"blade");
if (weapontype == 12) strcpy(wpnname,"sucker");
    ...
    if (IS_NPC(ch)) sprintf(buf,"%s's soul %s is lying
here.",ch->short_descr,wpnname);
...
*/
	...
	send_to_char("Runes: Fire, Air, Earth, Water, Dark, Light, Life, Death, Mind, Spirit, Mastery.\r\n", ch);
	return;}
	...
	send_to_char("Glyphs: Creation, Destruction, Summoning, Transformation, Transportation, Enhancement, Reduction.\r\n", ch);
	send_to_char("Control, Protection, Information.\r\n", ch);
	return;}
	itemtype = ITEM_SYMBOL;
	vn = 2;
	...
	send_to_char("Sigils: Self, Targeting, Area, Object.\r\n", ch);
	return;}
	itemtype = ITEM_SYMBOL;
	vn = 3;
...
void spell_limbo_scythe( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
