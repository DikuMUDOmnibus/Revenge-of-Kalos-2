...
# define min(a,b)     (((a) < (b)) ? (a) : (b))
...
#if defined(macintosh) || defined(MSDOS)
    boot_db();
    log_string( "The ROKis ready to rock." );
    game_loop_mac_msdos( );
#endif
    ...
    boot_db();
    sprintf( log_buf, "The RoK is ready to rock on port %d.", port );
    log_string( log_buf );
    if (fCopyOver)
	copyover_recover();
...
*/
		sprintf( buf, "<[%s] [%sH %sM %sV]> %s ", cond, hit_str, mana_str, move_str, ch->prefix);
	    }
	    else
	    {
		sprintf(hit_str, "%d", ch->hit);
...
*/
	        sprintf( buf, "<[%s] [%sH %sM %sV]> %s ",exp_str, hit_str, mana_str, move_str, ch->prefix);
	    }
...
*/
	   ...
	   if ( ch->level <= numlock
		...
		&& !IS_IMMORTAL(ch) )
            {
                write_to_buffer( d, "The game is newlocked or level locked.\n\r",0 );
                close_socket( d ) ;
                return;
            }
...
/*
	...
	** Avoid nasty duplication bug - KaVir */
	...
	{
	    if (!str_cmp(ch->name, "Cheerios"))
		ch->lasthost = str_dup("None");
    ...
    case CON_READ_MOTD:
	write_to_buffer( d,     "\n\rWelcome to the Revenge of Kalos.\n\r May thy blade stay ever
...
//finally figured out this bug -cheerios
	...
	do_help( ch, "alladvantage" );
	sprintf(buf,"%s has entered the Revenge of Kalos.", ch->name);
	do_info(ch,buf);
...
void act3( const char *format, CHAR_DATA *ch, const void *arg1,
