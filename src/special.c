...
const   struct  spec_type    spec_table[] =
{
    ...
    {	"spec_torcalta",		spec_clan_torcalta      },
...
SPEC_FUN *spec_lookup( const char *name )
{
    ...
    if ( !str_cmp( name, "spec_xkilla"            ) ) return spec_xkilla;
...
bool spec_xkilla( CHAR_DATA *ch )
{
	...
	{
	    act("$n's eyes glow bright orange for a  moment.",ch,NULL,NULL,TO_ROOM);
	    spell_teleport( skill_lookup( "heal" ), ch->level, ch, ch );
	    ch->spectype = ZOMBIE_REST;
	}
	else
	{
	...
	switch( number_percent() )
	{
		...
		break;
	    case 1:
		strcpy(buf,"You shall never win this battle!");
		do_say(ch,buf);
		break;
	    case 2:
		if (IS_NPC(victim))
		    sprintf(buf," You will be #b#1DESTROYED#n %s.",
victim->short_descr);
		else
		    sprintf(buf,"You will be #b#1DESTROYED#n %s.",victim->name);
		do_say(ch,buf);
	    case 3:
		if (IS_NPC(victim))
		    sprintf(buf,"%s shall pay for his arrogance!",victim->short_descr);
...
//very very annoying - made it so it happens less
	    if (speech == 2 && number_range(1,3) == 1)
			do_gsocial(ch,"grin");
	    if (speech == 3) do_emote(ch,"laughs at you.");
	    if (speech == 4)
	    {
		if(!IS_NPC(victim))
		    sprintf(buf,"What are you looking at %s.",victim->name);
		else
		    sprintf(buf,"What are you looking at %s.",victim->short_descr);
		do_say(ch,buf);
	    }

	    if (speech == 5)
	    {
		strcpy(buf,"Give me some money!");
		do_say(ch,buf);
	    }
	    if (speech == 6)
	    {
		if(!IS_NPC(victim))
		    sprintf(buf,"Hey, %s want to fight.",victim->name);
		else
		    sprintf(buf,"Hey, %s want to fight.",victim->short_descr);
		do_say(ch,buf);
	    }
	    if (speech == 7)
	    {
		if(!IS_NPC(victim))
		    sprintf(buf,"Hey look, it's the wimp, %s.",victim->name);
		else
		    sprintf(buf,"Hey look, it's the wimp, %s.",victim->short_descr);
		do_say(ch,buf);
	    }
	    if (speech == 8)
	    {
		if(!IS_NPC(victim))
		    sprintf(buf,"Laugh, %s.",victim->name);
		else
		    sprintf(buf,"Laugh, %s.",victim->short_descr);
		do_say(ch,buf);
	    }
	}
	/*
