...
//2nd affected
    ...
    fprintf( fp, "Xkaff	       %d\n",   ch->xkaff		);
    fprintf( fp, "Apowers      %d\n",   ch->xkilla->aliens      ); 
    fprintf( fp, "Dpowers      %d\n",   ch->xkilla->drows       );        
    fprintf( fp, "Dskills      %d\n",   ch->xkilla->dskills     );        
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    if ( xkilla_free == NULL )
    {
        ch->xkilla                      = alloc_perm( sizeof(*ch->xkilla) );
    }
    else
    {
        ch->xkilla                      = xkilla_free;
        xkilla_free                     = xkilla_free->next;
    }
...
void fread_char_old( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'A':
	    ...
	    KEY( "Apowers",	ch->xkilla->aliens,     fread_number( fp ) );   
	...
	case 'B':
	    ...
	    KEY( "Bounty",      ch->pcdata->bounty,     fread_number( fp ) );
            KEY( "Brood",       ch->brood,              fread_number( fp ) );
	    break;
	...
	case 'D':
	    ...
	    KEY( "Description",	ch->description,	fread_string( fp ) );
            KEY( "Dpowers",     ch->xkilla->drows,      fread_number( fp ) );
            KEY( "Dskills",     ch->xkilla->dskills,    fread_number( fp ) );            
	...
	case 'F':
            KEY( "Favorite",       ch->xkilla->favorite,              fread_number( fp ) ); 
	...
	case 'P':
	    ...
	    KEY( "Plr",		ch->xkilla->plr,	fread_number( fp ) );
	    break;
...
*/
	...
	case 'W':
	    ...
	    KEY( "Wimpy",	ch->wimpy,		fread_number( fp ) );
            KEY( "Warps",       ch->pcdata->warp,       fread_number( fp ) );
            KEY( "Warpcount",   ch->pcdata->warpcount,  fread_number( fp ) );
	    ...
	    break;
        case 'X':
            KEY( "Xkaff",        ch->xkaff,       fread_number( fp ) );
            break;   
	}
...
if( count >= MAX_ALIAS ) 	    	{
fread_to_eol( fp ); 	    		fMatch = TRUE;
break; 	    	} 	    	 	    	ch->pcdata->alias[count]
=	 str_dup( fread_word( fp ) );
ch->pcdata->alias_sub[count]  =	 fread_string( fp ); 	    	count++;
fMatch 			      =  TRUE; 	    	break;	 	    }
