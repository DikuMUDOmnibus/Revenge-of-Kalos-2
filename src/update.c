...
int mana_gain( CHAR_DATA *ch )
{
    ...
    {
	if (IS_CLASS(ch, CLASS_VAMPIRE)) return 0;
	gain = number_range( 50, 100 );

	if ((intamount = (get_curr_int(ch)+1)) > 1)
	{
	    switch ( ch->position )
	    {
		case POS_MEDITATING: gain *= intamount * 2;		break;
...
int move_gain( CHAR_DATA *ch )
{
	...
	if ((dexamount = (get_curr_dex(ch)+1)) > 1)
	{
	    switch ( ch->position )
	    {
		case POS_MEDITATING: gain *= dexamount * 2;	break;
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	case COND_THIRST:
	    if (IS_CLASS(ch, CLASS_VAMPIRE) && ch->hit > 0)
	    {
...
/*S*/
      if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_HYDRA) && ch->pcdata->powers[DISC_HYDRA_REGEN] == 1) {

         if (ch->position == POS_FIGHTING || ch->position == POS_SLEEPING
|| ch->position == POS_MEDITATING || ch->position == POS_RESTING) {
         if (ch->hit  < ch->max_hit) ch->hit += 125;
         if (ch->mana < ch->max_mana) ch->mana += 165;
         if (ch->move < ch->max_move) ch->move += 150;
                if (ch->loc_hp[6] > 0)
                {
         ...
         if (ch->hit > ch->max_hit) ch->hit += 2;
         if (ch->mana > ch->max_mana) ch->mana += 2;
         if (ch->move >ch->max_move) ch->move += 2;
         }
         update_pos(ch);
         continue;
         }
...
/*S*/
      if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_HYDRA) && ch->pcdata->powers[DISC_HYDRA_REGEN] == 2) 
         {
         int maxhitlower = 0;
         maxhitlower = ch->max_hit - 1;
         if (ch->hit > maxhitlower) {
         hydra_regen(ch, "");
         }
         ...
         else if (ch->position == POS_STANDING || ch->position == POS_SLEEPING || ch->position == POS_MEDITATING || ch->position == POS_RESTING ) {
         if (ch->mana < 75) {hydra_regen(ch,"");return;}
         if (ch->hit < ch->max_hit) ch->hit += 500;
         if (ch->hit < ch->max_hit) ch->mana -= 150;
         if (ch->move > 74) ch->move -= 75;
                if (ch->loc_hp[6] > 0)
                {
                    int sn = skill_lookup( "clot" );
                    (*skill_table[sn].spell_fun) (sn,ch->level,ch,ch);
         ...
         ch->hit += 2;
         if (ch->mana < ch->max_mana) ch->mana += 600;
         if (ch->move < ch->max_move) ch->move += 300;
         }
	   }
         update_pos(ch);
         continue;
	    ...
	    else if ( (IS_CLASS(ch, CLASS_DEMON) || IS_CLASS(ch, CLASS_DROWNEW))
			&& IS_HERO(ch) )
	    {
                if ( ch->hit < ch->max_hit || ch->mana < ch->max_mana ||
                    ch->move < ch->max_move )
		...
		if ( (IS_CLASS(ch, CLASS_DROWNEW) && ch->in_room != NULL &&
			ch->in_room->vnum == 100701) ||
		    (IS_CLASS(ch, CLASS_DEMON) && ch->in_room != NULL &&
...
void tick_update(void)
{
	...
	ch->exp += 1000;
        ch->pcdata->score[SCORE_TOTAL_XP] += 1000; 
    }
    return;
}
